  ...
  <div class="container mx-auto px-4 py-8 mb-30">
    ...
    <div v-if="currentUser.role === 'searcher'" class="bg-white rounded-lg shadow-md p-6">
      ...
      <div v-else>
        ...
        <div v-if="cards.length === 0" class="text-gray-500 text-center py-8">
          ...
          <div v-for="(response, index) in cards" :key="index"
            ...
            <div class="flex justify-between items-start">
              <div>
                ...
                <p class="text-gray-700 mt-2" v-if="response.message">
                  <span class="font-medium">Комментарий работодателя:</span> {{ response.message }}
                </p>
              </div>
              <div class="flex flex-col items-end">
                <span class="text-sm text-gray-500">{{ 'СНОВА ДАТА' }}</span>
                <button v-if="currentUser.role === 'employer'" @click="openResume(response.vacancy.id)"
    ...
    <div v-else-if="currentUser.role === 'employer'" class="space-y-6">
      <div v-if="loading" class="flex justify-center py-8">
        <div class="animate-spin rounded-full h-12 w-12 border-t-2 border-b-2 border-primary-500"></div>
      </div>
      ...
      <div v-else>
        ...
        <div v-else-if="currentUser.role === 'employer'" class="space-y-6">
          ...
          <div v-else>
            ...
            <div v-else>
              <div v-for="vacancy in cards" :key="vacancy.id" class="bg-white rounded-lg shadow-md p-6 mb-4">
                <div v-for="responder in vacancy.responses" :key="responder.id" class="mb-4 last:mb-0">
                  <div class="flex items-center justify-between">
                    <div>
                      <p class="text-gray-800">
                        Пользователь <span class="font-bold text-primary-500">{{ responder.name }} {{ responder.surname
                          }}</span>
                        откликнулся на вашу вакансию <span class="font-bold text-primary-500">{{ vacancy.name }}</span>
                      </p>
                    </div>
